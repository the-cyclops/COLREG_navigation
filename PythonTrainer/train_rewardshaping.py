...
# R6 setup: 
# gamma 0.995, lr 0.0003, ent 0.001, batchsize 256, logstd=0.0, gradclip 0.5 ( on critics too), costs and reward with scale 0.1
...
def main():
    ...
    for seed in SEEDS:
        ...
        rs_agent = RewardShapingPPOAgent(
            INPUT_SIZE, 
            ACTION_SIZE, 
            device=DEVICE, 
            gamma=GAMMA,
        ...
        if starting_step != 0:
            ...
            rs_agent.policy_net.load_state_dict(checkpoint['policy_state_dict'])
            rs_agent.value_net.load_state_dict(checkpoint['value_state_dict'])
            rs_agent.policy_opt.load_state_dict(checkpoint['policy_opt_state_dict'])
            rs_agent.value_opt.load_state_dict(checkpoint['value_opt_state_dict'])
            print(f"Model loaded, starting from step {starting_step}.")
        else:
            print(f"Start training on: {behavior_name}")

        rs_agent.set_train_mode()
        ...
        try:
            ...
            while s < TOT_STEPS: 
                ...
                while (len(memory_buffer.states) < ROLLOUT_SIZE or not end_episode):
                    ...
                    action_tensor, log_probabs = rs_agent.get_action(obs_tensor)
                    action_numpy = action_tensor.detach().cpu().numpy()
                    action_tuple = ActionTuple()
                    action_tuple.add_continuous(action_numpy)

                    with torch.no_grad():
                        mean, _, std = rs_agent.policy_net(obs_tensor)
                ...
                log_dict = rs_agent.update(rollouts=rollout_buffer, 
                ...
                violated = log_dict.get('violated_rules', [])
                writer.add_scalar("Violations/R1", int("R1" in violated), s)
                writer.add_scalar("Violations/R2", int("R2" in violated), s)
                writer.add_scalar("Violations/R6", int("R6" in violated), s)
                writer.add_scalar("Violations/Count", len(violated), s)
                mode_id = 0 if "NOMINAL" in mode else (1 if "SINGLE" in mode else 2)
                writer.add_scalar("Training/Mode_State (0=Nom, 1=Single, 2=Multi)", mode_id, s)
                ...
                checkpoint = {
                        'step': s,
                        'policy_state_dict': rs_agent.policy_net.state_dict(),
                        'value_state_dict': rs_agent.value_net.state_dict(),
                        'policy_opt_state_dict': rs_agent.policy_opt.state_dict(),
                        'value_opt_state_dict': rs_agent.value_opt.state_dict(),
                ...
                if n_updates % EVAL_INTERVAL == 0 and s >= START_SAFETY:
                    ...
                    mean_eval_return, total_r1_robustness, total_r2_robustness, total_r6_robustness = evaluate_model(eval_seed=EVAL_SEED, agent=rs_agent, colreg_handler=colreg_handler, RTAMT=RTAMT, eval_env=eval_env, eval_env_params=eval_env_params)
